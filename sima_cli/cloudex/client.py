...
def _forwarder_failure(log_path):
    ...
    mappings = {
        ...
        "iproute2 is unavailable": "iproute2 is required to create the encrypted tunnel",
        "kernel wireguard is unavailable and wireguard-go is unavailable": "this kernel lacks WireGuard support and the wireguard-go fallback is not installed",
        "create userspace wireguard interface": "the wireguard-go fallback could not create the encrypted tunnel interface",
        "create wireguard interface": "the host could not create a kernel or userspace WireGuard interface",
        "assign wireguard address": "the host could not assign the CloudEx tunnel address",
        "start wireguard interface": "the host could not activate the CloudEx tunnel interface",
        "add wireguard route": "the host could not add the CloudEx device route",
        "configure wireguard interface": "the host could not configure the CloudEx tunnel interface",
        "configure wireguard peer": "the host could not configure the CloudEx WireGuard peer",
        "bind wireguard bridge": "the host could not bind the local CloudEx packet bridge",
        "trigger wireguard handshake": "the host could not send the initial CloudEx handshake packet",
        "wireguard key setup failed": "the host could not generate a WireGuard key pair",
