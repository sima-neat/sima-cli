...
def ensure_codex_vscode_extension_installed(
    ...
    install_script = (
        ...
        + shlex.quote(
            ...
            f"extensions_dir = Path({extensions_dir!r})\n"
            f"extension_id = {extension_id!r}\n"
            "publisher, name = extension_id.split('.', 1)\n"
            "for package_json in extensions_dir.glob(f'{publisher}.{name}-*/package.json'):\n"
            "    try:\n"
            "        data = json.loads(package_json.read_text(encoding='utf-8'))\n"
            "    except Exception:\n"
            "        continue\n"
            "    contributes = data.setdefault('contributes', {})\n"
            "    containers = contributes.setdefault('viewsContainers', {})\n"
            "    activity = [c for c in containers.get('activitybar', []) if c.get('id') != 'codexViewContainer']\n"
            "    activity.append({\n"
            "        'id': 'codexViewContainer',\n"
            "        'title': 'Codex',\n"
            "        'icon': 'resources/blossom-white.svg',\n"
            "    })\n"
            "    containers['activitybar'] = activity\n"
            "    containers.pop('secondarySidebar', None)\n"
            "    views = contributes.setdefault('views', {})\n"
            "    primary_views = views.get('codexViewContainer') or [{\n"
            ...
            "        view.pop('when', None)\n"
            "    views['codexViewContainer'] = primary_views\n"
            "    views.pop('codexSecondaryViewContainer', None)\n"
